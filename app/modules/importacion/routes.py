...
#         # Validar columnas requeridas
#         columnas_requeridas = ['codigo_curso', 'nombre_curso', 'semestre','ciclo']
#         for columna in columnas_requeridas:
#             if columna not in df.columns:
#                 flash(f'Columna requerida faltante: {columna}', 'danger')
#                 return redirect(url_for('importacion.index'))
...
def importar_cursos():
    ...
    try:
        ...
        # Validar columnas requeridas
        columnas_requeridas = ['codigo_curso', 'nombre_curso', 'creditos', 'semestre', 'ciclo']
        for columna in columnas_requeridas:
            if columna not in df.columns:
                flash(f'Columna requerida faltante: {columna}', 'danger')
                return redirect(url_for('importacion.index'))
        ...
        for _, fila in df.iterrows():
            ...
            # Buscar ciclo por código (2025-1, 2025-2)
            ciclo = Ciclo.query.filter_by(codigo_ciclo=fila['ciclo']).first()
            if not ciclo:
                flash(f"❌ No existe el ciclo '{fila['ciclo']}' para el curso {fila['codigo_curso']}", 'danger')
                continue
            
            # Buscar si el curso ya existe por código
            curso = Curso.query.filter_by(codigo_curso=fila['codigo_curso']).first()
            
            if curso:
                # Actualizar curso existente
                curso.nombre_curso = fila['nombre_curso']
                curso.creditos = fila['creditos']
                curso.semestre = fila['semestre']
